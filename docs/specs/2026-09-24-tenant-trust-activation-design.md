...
- It requires an **interactive Azure user context**: it calls `az account show` and asserts the authenticated principal's `UserType` is `user` (never a service principal) and its `UserPrincipalName` matches the reviewed `AdminUpn`. `az account get-access-token` is a separate call, used only to obtain bearer tokens for the Microsoft Graph and Azure DevOps API requests. This is why it can only ever be run by a human with their own `az login` — it is designed for exactly that, not for a GitHub Actions workflow.
